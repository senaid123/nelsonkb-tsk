      ...
      {(["car1", "car2"] as const).map((id, i) => {
        ...
        return (
          ...
          >
            ...
            {active && (
              <div
                className="px-4 pb-3 pt-1 flex items-center justify-between gap-3"
                style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}
              >
                <div>
                  <p className="text-[8px] tracking-[0.2em] text-white/30 mb-0.5">Y ROTATION</p>
                  <p className="text-[13px] font-mono font-semibold text-white/70">{rotDeg}°</p>
                </div>
                <button
                  onClick={() => onResetRotation(id)}
                  className="text-[8px] tracking-[0.15em] px-2.5 py-1.5 transition-all duration-150"
                  style={{
